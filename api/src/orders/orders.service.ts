...
import { isVertical, mayList } from '../catalog/verticals';
import { RAILS } from './payments';
import { cashAllowed } from './cash';
...
export class OrdersService {
  ...
  create(userId: string, dto: CreateOrderDto, idempotencyKey: string) {
    ...
    /* payments.ts, wired in 16 Sep 2026. A rail with no credentials refuses here
       by name, exactly as the checkout screen does. Until now the API accepted an
       Orange Money order that no connector could take, and the customer found out
       never. */
    const rail = RAILS[dto.payment_method];
    if (rail && rail.blockedBy) {
      throw new HttpException(
        { code: 'RAIL_NOT_OPEN', message: `${rail.label} is not switched on yet: ${rail.blockedBy}` },
        409,
      );
    }
    ...
    /* THE RULES LAYER, WIRED IN. Until 16 September 2026 verticals.ts was tested
       sixteen ways and never called from this path, so a pharmacy order could be
       placed through the API while the home page said pharmacy was shut. A shop
       whose kind is not open cannot take an order, whatever the screen showed. */
    if (isVertical(merchant.type)) {
      const gate = mayList(merchant.type);
      if (!gate.listable) {
        throw new HttpException(
          { code: 'NOT_OPEN', message: `${merchant.name} cannot take orders yet: ${gate.reason}` },
          403,
        );
      }
    }
    ...
    const orderItems = dto.items.map((it) => {
      ...
      /* Found by Codex on 16 Sep 2026: `!qty || qty < 1` let Infinity, 2.5 and
         `true` through, and Infinity made the order total null. A quantity is a
         whole number, and nobody orders a thousand of anything on a bike. */
      if (!Number.isInteger(it.qty) || it.qty < 1 || it.qty > 999) {
        throw new BadRequestException(`Quantity for ${found.name} must be a whole number from 1 to 999.`);
      }
      return { item_id: found.id, name: found.name, qty: it.qty, price_bwp: found.price_bwp };
    });

    /* Found by an outside review on 16 Sep 2026: this was goods only, so a P20
       item at a merchant charging P25 delivery returned P20 and the customer was
       quoted twenty five pula short of what they owe. Goods, fee and the payable
       total are now three separate numbers, because conflating them is how a
       delivery fee goes uncollected. */
    const goods_bwp = Math.round(
      orderItems.reduce((sum, it) => sum + it.price_bwp * it.qty, 0) * 100,
    ) / 100;
    const delivery_fee_bwp = Number(merchant.deliveryFee) || 0;
    const total_bwp = Math.round((goods_bwp + delivery_fee_bwp) * 100) / 100;
    if (!Number.isFinite(total_bwp) || total_bwp <= 0) {
      throw new BadRequestException('The order total did not add up. Nothing was placed.');
    }
    /* cash.ts, wired in 16 Sep 2026. The P1000 cap has been enforced by the cash
       engine since brick 18 because a rider carrying more is a target. The API
       never asked it. */
    if (dto.payment_method === 'cash') {
      const verdict = cashAllowed(Math.round(total_bwp * 100));
      if (!verdict.allowed) throw new BadRequestException(verdict.says);
    }
    ...
    /* Found by Codex on 16 Sep 2026: the store flushes on a two second timer, and
       this used to return the order to the customer before that flush. A crash
       in those two seconds lost the order AND its idempotency key, so the retry
       placed a second order with a second id. An order is not placed until it is
       on disk, so the write happens here, before anyone is told. */
    this.store.flush();
