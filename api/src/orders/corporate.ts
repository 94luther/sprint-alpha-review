...
export interface CorporateOrder {
  ...
  placed_at: string;
  description: string;
  /** A cancelled order must stop eating the limit. Absent means placed. */
...
export function statement(
  ...
  /* Half open, found by an outside review on 16 Sep 2026. A date with no time is
     midnight at the START of that day, so an inclusive end silently dropped every
     order placed on the last day of the month, and an order sitting exactly on a
     boundary two periods shared was counted in both. */
...
/**
 * A date with no time is midnight at the START of that day. Found by an outside
 * review on 16 Sep 2026: a statement ending 2026-09-30 dropped every order placed
 * on the 30th, and an inclusive end double counted an order sitting exactly on a
 * boundary shared with the next period. Periods are half open now: from the start
 * of `from`, up to but not including the start of the day after `to`.
 */
export function endOfDay(dateOnlyOrISO: string): number {
  const t = Date.parse(dateOnlyOrISO);
  if (Number.isNaN(t)) throw new CorporateError('That is not a date: ' + dateOnlyOrISO);
  if (/^\d{4}-\d{2}-\d{2}$/.test(dateOnlyOrISO)) return t + 24 * 60 * 60 * 1000;
  return t;
}
...
export function outstanding(orders: CorporateOrder[], accountId: string, paidUpTo?: string): number {
  ...
  return orders
    .filter((o) => o.account_id === accountId && o.state !== 'cancelled' && Date.parse(o.placed_at) > cutoff)
...
/* ---------------------------------------------------------------------------
   RESERVING THE CREDIT. Found by both reviews on 16 September 2026.

   canOrder was a pure check with no caller. Even wired in, it reserved nothing,
   so two orders reading the same balance both passed and the account went over.
   This is the check and the reservation in ONE synchronous step: there is no
   await between reading what is owed and recording the new order, so in a single
   Node process nothing can slip between them. A Postgres version must do the
   same in one statement, or the race comes straight back.
   --------------------------------------------------------------------------- */
export function placeOnAccount(
  book: CorporateOrder[],
  account: CorporateAccount,
  accountId: string,
  amount: number,
  ...
  description: string,
  paidUpTo?: string,
): { placed: boolean; says: string; order: CorporateOrder | null } {
  if (!Number.isInteger(amount) || amount <= 0) {
    return { placed: false, says: 'An amount on account is whole thebe, more than nothing.', order: null };
  }
  const owed = outstanding(book, accountId, paidUpTo);
  const verdict = canOrder(account, owed, amount, costCentre);
  if (!verdict.allowed) return { placed: false, says: verdict.says, order: null };
  const order: CorporateOrder = {
    order_id: orderId,
  ...
  book.push(order);
  return { placed: true, says: verdict.says, order };
}

/** A cancelled order gives its credit back. It stays in the book, marked. */
export function cancelOnAccount(book: CorporateOrder[], orderId: string): boolean {
  const o = book.find((x) => x.order_id === orderId);
  if (!o || o.state === 'cancelled') return false;
  o.state = 'cancelled';
  return true;
}
