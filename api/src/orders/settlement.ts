...
export interface PayoutLine {
  ...
  deductions: number;
  /** Sprint's agreed share of the goods, already taken out of `goods`. */
  commission: number;
...
export interface Payout {
  ...
  /** Sprint's commission on the goods for the day. Zero unless terms were agreed. */
  sprint_commission: number;
...
export function settleOne(o: SettleableOrder, terms: MerchantTerms | null = null): {
  ...
  /* Found by an outside review on 16 Sep 2026. This guard rejected negatives and
     let everything else through, so two merchants owed 100.4 thebe each exported
     as P1.00 apiece against a total of P2.01. Money is whole thebe or it is not
     money. */
  if (!Number.isFinite(o.goods) || !Number.isFinite(o.delivery_fee)) {
    throw new SettlementError('An order must carry real amounts. ' + o.order_id + ' does not.');
  }
  if (!Number.isInteger(o.goods) || !Number.isInteger(o.delivery_fee)) {
    throw new SettlementError('Money is whole thebe. ' + o.order_id + ' has a fraction of one on it.');
  }
 ...
 */
export function dayPayout(
  day: string,
  orders: SettleableOrder[],
  agreed: boolean,
  /* Found by Codex on 16 Sep 2026: this used to call settleOne without terms, so
     an agreed commission was computed perfectly by commission.ts and then dropped
     on the floor here. A P200 basket at 15 percent paid the merchant P200. */
  terms: ((merchantId: string) => MerchantTerms | null) | null = null,
): Payout {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(day)) throw new SettlementError('A payout needs a day, as 2026-09-13.');

  /* Found by an outside review on 16 Sep 2026, two faults in one loop.
     FIRST, the lines were keyed by merchant alone, so the first order supplied the
     rail and the funding flag for every later order of that merchant. One cash
     order and one MyZaka order funded P0 or P400 depending purely on which came
     first, when the wallet portion is P200 either way. The key now carries the
     rail, so a merchant paid two ways gets one line per way.
     SECOND, nothing ever looked at order_id, so the same order twice in a batch
     was paid twice. A replayed or double joined batch is the normal way that
     happens, and it is silent. */
  const byLine = new Map<string, PayoutLine>();
  const seen = new Set<string>();
  let fees = 0, absorbed = 0, commission = 0;

  for (const o of orders) {
    if (seen.has(o.order_id)) {
      throw new SettlementError(
        'Order ' + o.order_id + ' is in this payout twice. Refusing to pay it twice.',
      );
    }
    seen.add(o.order_id);
    const s = settleOne(o, terms ? terms(o.merchant_id) : null);
    fees += s.sprint_fee;
    absorbed += s.sprint_absorbs;
    commission += s.commission;

    const key = o.merchant_id + '|' + o.rail;
    const line = byLine.get(key) ?? {
      merchant_id: o.merchant_id,
      merchant_name: o.merchant_name,
      orders: 0, goods: 0, deductions: 0, perishable_loss: 0, net: 0, commission: 0,
      rail: o.rail,
  ...
  const lines = [...byLine.values()].map((l) => {
    ...
    l.says = l.funded_by_sprint
      ...
      : SELF_SETTLING.includes(l.rail)
        ? `${pula(l.net)} to ${l.merchant_name}, settled by ${railName(l.rail)} itself, nothing for Sprint to move`
...
/**
 * The file somebody hands to the bank.
 *
 * Found by an outside review on 16 Sep 2026: this used to export an unapproved
 * payout byte for byte identical to an approved one, so the one warning that
 * mattered disappeared at exactly the moment money was about to move. A person
 * reading the file could not tell. It now refuses, and a draft must be asked for
 * deliberately and says so on its own first line.
 */
export function payoutCsv(p: Payout, draft = false): string {
  if (!p.agreed && !draft) {
    throw new SettlementError(
      'This payout is not agreed, so it cannot be exported for payment. Ask for a draft if you want to look at it.',
    );
  }
  const header = p.agreed
    ? ''
    : 'DRAFT, NOT PAYABLE. The fault model is still a proposal.\n';
  ...
  return header + ([head, ...rows, foot].join('\n') + '\n');
}
